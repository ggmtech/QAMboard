...
install.packages("tinytax", dependencies = TRUE)
...
library(here)
rmd_filename <- str_remove( knitr::current_input(),  "\\.Rmd"  ) # get filename striping file extension
...
# To utilize the above script in an RMarkdown file,
# simply insert the code below as a chunk into the RMarkdown file. 
library(here)
source(here::here("rmd_config.R"))

# For a Jekyll blog, you’ll also want to include certain YAML header 
---
  layout: post
...
output: 
  md_document:
  pandoc_args: ["--wrap=none"] #prevent inserting extra line breaks into Rmd  
variant: gfm
preserve_yaml: TRUE
---
...
# load(installscript.R)
###  Install all Package names   ##################
packages <- c( "tidyverse" 
               , "magrittr", "ggplot2", "readxl", "dplyr", "tidyr", "reshape2",  "stringr", "forcats",
               "tinytex", "knitr", 
               "here",  "pander", "ggpubr", 
               #"summarytools",
               "ggfortify", "ggstatsplot",
               "googlesheets", "googledrive",
               "FactoMineR", "factoextra","corrplot", "scales", "DT" ,
               # "pwr", "psy", "psych", "car", "doBy", "imputeMissings", "RcmdrMisc", "questionr", 
               # "vcd", "multcomp", "KappaGUI", "rcompanion",  
               # "ltm", "goeveg",  "FSA", "MASS", "nlme", 
               #  "ordinal", "lmtest", "dslabs",  "assist", 
               #   "styler", "remedy", "snakecaser", "addinslist", 
               # "esquisse",     "funModeling", 
               #  "cluster", "abind"
               "lubridate"
)
...
# get suggetsion on ... from github : gh_suggest("cats")
# gh_suggest("cats")
# gh_search_packages("lasso")  # search packages
# gh_show_source("mutate", repo = "dplyr")  # show source code
# list gh_update_package_list()
# list http://cran.r-project.org/web/packages/
# list http://www.bioconductor.org/packages/release/Software.html

#githubinstall("PackageName") # does not need installer name
...
# devtools::install_github("hadley/babynames") # from GitHub, # install_local() from a local file,
# detach("package:babynames", unload=TRUE)

# install_url() from a URL, and
# help(package = "babynames")  # help
# a package is a like a book, you use library() to check a package out of the library 

#install.packages("pacman")
#?pacman::p_load( `packages` , install = TRUE, )  #ggplot2, tidyr, dplyr)
pacman::p_load( `packages` ,      # char vector else list all packages separetaly
                # install = TRUE, 
                # update = getOption("pac_update"), #update packages. Default "pac_update" in .Rprofile.
                character.only = TRUE  # if package name is a vector
)
# pacman::p_delete() , p_functions, p_help	, p_info
# vignette(package = "ggplot2")
# replace help of Rdocumention site
...
## librarian::shelf(packages ) #ggplot2, DesiQuintans/desiderata, pander)
##################
...
# googledrive::drive_get("gapminder") %>% sheets_read()
# ( ss <- sheets_create("fluffy-bunny", sheets = list(flowers = head(iris)))) # create new sheet
# head(mtcars) %>%  sheets_write(ss, sheet = "autos")  # overwrite
# sheets_edit() and sheets_append()
sheets_deauth()

library(googledrive)
#x library(googlesheets)   
...
RDSOvd <- read_sheet("1FlEhwZ3Yn-lEs7Xb5qQXAlRQaqcQaKipK-XGU7wrHhg")
class(RDSOvd)
unclass(RDSOvd)
as_sheets_id(RDSOvd) # ? error

sheets_find()       # short cut for drive_find(type = "spreadsheet")  # yes after autho listed
#?sheets_browse(RDSOvd)
# good list sheets !
sheets_get("1FlEhwZ3Yn-lEs7Xb5qQXAlRQaqcQaKipK-XGU7wrHhg")

ss = "1FlEhwZ3Yn-lEs7Xb5qQXAlRQaqcQaKipK-XGU7wrHhg"
sheet = "FinalMasterList31march"
read_sheet( 
    ss,  # that identifies a Google Sheet: its file ID, a URL from which we can recover the ID,
    sheet = NULL,  #Sheet to read,
    range = NULL,
    col_names = TRUE,  # first row as columnnames
    col_types = NULL,  # NULL to guess all
    na = "",
    trim_ws = TRUE,
    skip = 2,     # rows to skip
    # n_max = Inf,
    # guess_max = min(1000, n_max),
    .name_repair = "unique"  # ensure name non empty and unique
)  -> RDSOvdData
